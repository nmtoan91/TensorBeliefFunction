...
try:
    for i in tqdm(range(2,n),'TensorMassFunction_CSC'):
        frame,dicts,array = GenerateSingularrameOfDiscernment(i)
        tmp = TensorMassFunction_CSC.SetFrame(frame)
        frame2,dicts2,array2 = GenerateSingularrameOfDiscernment(i)
        time1 = time.time()
        tm1 = TensorMassFunction_CSC(array)
        tm2 = TensorMassFunction_CSC(array2)
