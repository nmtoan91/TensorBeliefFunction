...
from tensords.tensords_csr import TensorMassFunction_CSR
...
try:
    for i in tqdm(range(2,n),'matrix_csr'):
        ...
        tmp = TensorMassFunction_CSR.SetFrame(frame)
        frame2,dicts2,array2 = GenerateFullFrameOfDiscernment(i)
        time1 = time.time()
        tm1 = TensorMassFunction_CSR(dicts)
        tm2 = TensorMassFunction_CSR(dicts2)
